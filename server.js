...
async function resolveSessionTiming(session) {
  if (!session) return null;
  const scheduled = validDate(session.scheduled_at);
  if (!session.started_at && scheduled && scheduled.getTime() <= Date.now()) {
    session = await db.markSessionStarted(session.id, scheduled.toISOString());
  }
  const started = validDate(session.started_at);
  const ended = validDate(session.ended_at);
  const status = ended ? 'ended' : started ? 'running' : 'lobby';
  return {
    session,
    public: {
      code:session.code,title:session.title,venue:session.venue,isOpen:session.is_open,
      status,scheduledAt:scheduled?.toISOString() || null,startedAt:started?.toISOString() || null,
      endedAt:ended?.toISOString() || null,serverNow:nowIso(),
      startsInSeconds:status === 'lobby' && scheduled ? Math.max(0,Math.ceil((scheduled.getTime()-Date.now())/1000)) : 0,
    },
  };
}
...
function safePlayer(player, progress) {
  return {
    ...
    progress:{
      ...
      discoveries:progress.discoveries,notes:progress.notes,theory:progress.theory,updatedAt:progress.updatedAt,
    },
  };
}
...
function ensureProgressAllowed() { return null; }
...
app.get('/api/health', (req,res) => res.json({ ok:true,db:process.env.DATABASE_URL ? 'postgres':'local-json',version:'3.3.0',cases:GAME.caseOrder.length }));

app.get('/api/config', async (req,res) => {
  const code = normalizeSessionCode(req.query.session || 'OURMOMS');
  const rawSession = await db.getSessionByCode(code);
  const timing = await resolveSessionTiming(rawSession);
  res.json({
    title:GAME.app.title,subtitle:GAME.app.subtitle,defaultSession:'OURMOMS',
    session:timing?.public || null,
...
app.post('/api/register', async (req,res) => {
  try {
    ...
    let session = await db.getSessionByCode(values.sessionCode);
    ...
    const sessionTiming = await resolveSessionTiming(session); session = sessionTiming.session;
    ...
    return res.json({
      ...
      session:sessionTiming.public,player:safePlayer(player,progress),game:publicGame(player,progress),accusation:null,
    });
  } catch (error) {
...
app.post('/api/resume', async (req,res) => {
  ...
  const sessionTiming = await resolveSessionTiming(session); session = sessionTiming.session;
  return res.json({ token:sign({type:'player',playerId:player.id,sessionId:session.id}),session:sessionTiming.public,player:safePlayer(saved,progress),game:publicGame(saved,progress,accusation),accusation:decorateAccusation(accusation,caseFor(saved),saved) });
});
...
app.get('/api/me', authPlayer, async (req,res) => {
  ...
  let session = await db.getSessionById(player.session_id);
  const { progress } = prepareProgress(player);
  const saved = await persistPlayer(player,progress);
  const accusation = await db.getAccusation(player.id);
  const sessionTiming = await resolveSessionTiming(session); session = sessionTiming.session;
  return res.json({ session:sessionTiming.public,player:safePlayer(saved,progress),game:publicGame(saved,progress,accusation),accusation:decorateAccusation(accusation,caseFor(saved),saved) });
});

app.get('/api/game', authPlayer, async (req,res) => {
  const player = await db.getPlayer(req.auth.playerId);
  if (!player || player.session_id !== req.auth.sessionId) return res.status(404).json({ error:'Player not found.' });
  const sessionTiming = await resolveSessionTiming(await db.getSessionById(player.session_id));
  const { progress } = prepareProgress(player);
  const accusation = await db.getAccusation(player.id);
  await persistPlayer(player,progress);
  return res.json({ session:sessionTiming.public,game:publicGame(player,progress,accusation) });
});
...
app.post('/api/action', authPlayer, async (req,res) => {
  try {
    ...
    if (!player || player.session_id !== req.auth.sessionId) return res.status(404).json({ error:'Player not found.' });
    const sessionTiming = await resolveSessionTiming(await db.getSessionById(player.session_id));
    if (sessionTiming.public.status !== 'running') return res.status(409).json({ error:'The investigation has not started yet. Wait in the lobby for the official start time.', session:sessionTiming.public });
...
app.post('/api/accusation', authPlayer, async (req,res) => {
  try {
    ...
    if (!player || player.session_id !== req.auth.sessionId) return res.status(404).json({ error:'Player not found.' });
    const sessionTiming = await resolveSessionTiming(await db.getSessionById(player.session_id));
    if (sessionTiming.public.status !== 'running') return res.status(409).json({ error:'The investigation has not started yet.' });
...
function hostPlayer(player) {
  ...
  let status = player.score != null ? 'Completed' : 'Playing';
...
app.post('/api/host/session', authHost, async (req,res) => {
  ...
  const scheduledDate = validDate(req.body?.scheduledAt);
  ...
  if (!scheduledDate) return res.status(400).json({ error:'Choose a valid game date and start time.' });
  try {
    ...
    return res.json({ session, token:sign({type:'host',sessionId:session.id},12) });
  } catch (error) {
...
app.get('/api/host/session', authHost, async (req,res) => {
  let session = await db.getSessionById(req.auth.sessionId);
  if (!session || session.archived_at) return res.status(404).json({ error:'Session not found or has been cleared.' });
  const sessionTiming = await resolveSessionTiming(session); session = sessionTiming.session;
  const raw = await db.listPlayers(session.id);
  const players = raw.map(hostPlayer);
  const counts = { registered:players.length,playing:players.filter((p)=>p.status==='Playing').length,waiting:sessionTiming.public.status==='lobby'?players.length:0,help:players.filter((p)=>p.status==='May need help').length,completed:players.filter((p)=>p.status==='Completed').length };
  return res.json({ session:{...session,...sessionTiming.public},counts,players,leaderboard:await db.leaderboard(session.id),versionDistribution:await db.versionCounts(session.id),activeSessions:await db.listActiveSessions() });
});


app.post('/api/host/switch-session', authHost, async (req,res) => {
  const session = await db.getSessionById(String(req.body?.sessionId || ''));
  if (!session || session.archived_at) return res.status(404).json({ error:'Active game not found.' });
  return res.json({ token:sign({type:'host',sessionId:session.id},12),session });
});

app.post('/api/host/archive-session', authHost, async (req,res) => {
  const session = await db.getSessionById(req.auth.sessionId);
  if (!session || session.archived_at) return res.status(404).json({ error:'Game not found or already cleared.' });
  const archived = await db.archiveSession(session.id);
  return res.json({ ok:true, archivedAt:archived.archived_at });
});
...
app.post('/api/host/schedule', authHost, async (req,res) => {
  const scheduledDate = validDate(req.body?.scheduledAt);
  if (!scheduledDate) return res.status(400).json({ error:'Choose a valid game date and start time.' });
  if (scheduledDate.getTime() < Date.now() - 60_000) return res.status(400).json({ error:'The game start time cannot be in the past.' });
  const session = await db.setSessionSchedule(req.auth.sessionId, scheduledDate.toISOString());
  const timing = await resolveSessionTiming(session);
  return res.json({ session:{...timing.session,...timing.public} });
});
...
db.init().then(()=>app.listen(PORT,()=>console.log(`Reunion mystery v3.3.1 running on http://localhost:${PORT}`))).catch((error)=>{ console.error(error); process.exit(1); });
