...
app.get('/api/host/session', authHost, async (req,res) => {
  ...
  return res.json({ session:{...session,...sessionTiming.public},counts,players,leaderboard:await db.leaderboard(session.id),versionDistribution:await db.versionCounts(session.id) });
});
...
db.init().then(()=>app.listen(PORT,()=>console.log(`Reunion mystery v3.3.0 running on http://localhost:${PORT}`))).catch((error)=>{ console.error(error); process.exit(1); });
