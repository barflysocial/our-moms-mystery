...
function chapterTasks(player, progress, accusation = null) {
  ...
  switch (progress.chapter) {
    ...
    case 'C05': return [{ text: 'Lock your final accusation', done: Boolean(accusation) }];
    default: return [];
  }
}
...
function advanceIfReady(player, progress, events) {
  ...
  if (next) {
    ...
    events.push({ type: 'chapter', message: `${next.title} unlocked.` });
  }
}
...
function safePlayer(player, progress) {
  return {
    ...
    progress:{
      chapter:progress.chapter,chaptersCompleted:progress.chaptersCompleted,evidenceViewed:progress.evidenceViewed,
...
function decorateAccusation(accusation, caseData) {
  ...
  return {
    ...
    solution:{ killer:caseData.solution.killerName,motive:caseData.solution.motive,weapon:caseData.solution.weapon,keyClue:caseData.solution.clue,sequence:caseData.solution.sequence,confession:caseData.solution.confession },
  };
}
...
app.get('/api/health', (req,res) => res.json({ ok:true,db:process.env.DATABASE_URL ? 'postgres':'local-json',version:3,cases:GAME.caseOrder.length }));
...
app.post('/api/resume', async (req,res) => {
  ...
  return res.json({ token:sign({type:'player',playerId:player.id,sessionId:session.id}),session:{code:session.code,title:session.title,venue:session.venue},player:safePlayer(saved,progress),game:publicGame(saved,progress,accusation),accusation:decorateAccusation(accusation,caseFor(saved)) });
});
...
app.get('/api/me', authPlayer, async (req,res) => {
  ...
  const saved = await persistPlayer(player,progress);
  const accusation = await db.getAccusation(player.id);
  return res.json({ session:{code:session.code,title:session.title,venue:session.venue},player:safePlayer(saved,progress),game:publicGame(saved,progress,accusation),accusation:decorateAccusation(accusation,caseFor(saved)) });
});
...
app.post('/api/accusation', authPlayer, async (req,res) => {
  try {
    ...
    const saved = await persistPlayer(player,progress);
    return res.json({ accusation:decorateAccusation(accusation,caseData),player:safePlayer(saved,progress),game:publicGame(saved,progress,accusation) });
  } catch (error) {
...
db.init().then(()=>app.listen(PORT,()=>console.log(`Reunion mystery v3 running on http://localhost:${PORT}`))).catch((error)=>{ console.error(error); process.exit(1); });
