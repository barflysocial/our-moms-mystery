...
async function switchHostSession(sessionId){
  const data=await api('/api/host/switch-session',{method:'POST',host:true,body:JSON.stringify({sessionId})});
  state.hostToken=data.token;sessionStorage.setItem('hostToken',data.token);syncSession(data.session.code);state.hostData=null;await loadHost();
}
async function clearCurrentGame(){
  const code=state.hostData?.session?.code||'';
  const answer=prompt(`Clear game ${code}? Type CLEAR to confirm. This hides the game and closes player access but keeps reports.`);
  if(answer!=='CLEAR')return;
  await api('/api/host/archive-session',{method:'POST',host:true,body:'{}'});
  state.hostToken=null;state.hostData=null;sessionStorage.removeItem('hostToken');toast('Game cleared from Active Games');renderHost();
}
