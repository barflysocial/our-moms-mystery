...
function shell(content){
  ...
  return `<div class="app-shell">
    <header class="topbar"><div class="brandrow"><div class="mini-crest">06</div><div class="brandtext"><b>Reunion at Our Mom's</b><span>${esc(investigatorLabel())} · ${esc(state.game.profile.publicLabel)}</span></div><button class="iconbtn" onclick="togglePause()" aria-label="${paused?'Resume':'Pause'} case">${paused?'▶':'Ⅱ'}</button></div>
...
function objectiveButton(obj){
  if(!obj)return '';
  const actions={
    briefing:`openBriefing('${obj.id}')`,evidence:`openEvidence('${obj.id}')`,casefiles:`openFiles('${obj.id}')`,suspect:`openSuspect('${obj.id}')`,leads:`go('leads')`,deduction:`openDeduction('${obj.id}')`,accuse:`go('accuse')`,resume:'togglePause()',wait:`openFiles('evidence')`,home:`go('files')`
  };
