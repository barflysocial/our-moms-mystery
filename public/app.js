...
function applyPayload(data){
  if(data.session?.code)syncSession(data.session.code); if(data.player)state.player=data.player; if(data.game)state.game=data.game;
...
function shell(content){
  const p=progress(); const paused=p.paused;
  return `<div class="app-shell">
    <header class="topbar"><div class="brandrow"><div class="mini-crest">06</div><div class="brandtext"><b>Reunion at Our Mom's</b><span>${esc(state.player.displayName)} · ${esc(state.game.profile.publicLabel)}</span></div><button class="iconbtn" onclick="togglePause()" aria-label="${paused?'Resume':'Pause'} case">${paused?'▶':'Ⅱ'}</button></div>
    <div class="progress"><span style="width:${state.game.progressPercent}%"></span></div></header>
    ${paused?`<div class="pausebar"><b>Case paused.</b> Review discovered files or resume when your table is ready.</div>`:''}
...
function objectiveButton(obj){
  if(!obj)return '';
  const actions={
    evidence:`openEvidence('${obj.id}')`,casefiles:`openFiles('${obj.id}')`,suspect:`openSuspect('${obj.id}')`,leads:`go('leads')`,deduction:`openDeduction('${obj.id}')`,accuse:`go('accuse')`,resume:'togglePause()',wait:`openFiles('evidence')`,home:`go('files')`
  };
...
function renderHome(){
  ...
  return shell(`<section class="chapter-kicker"><span>${esc(currentChapter().title)}</span><b>${esc(currentChapter().headline)}</b></section>
...
function renderFiles(){
  ...
  if(state.filesTab==='evidence'){
    ...
    return shell(`${tabs}<div class="pagehead"><h1>Case Files</h1><p>Only available evidence is shown. More appears as your investigation progresses.</p></div>
      ${available.length?`<h2 class="section-title">Available evidence</h2><div class="cardlist">${available.map((e)=>`<button class="filecard ${p.paused?'disabled':''}" ${p.paused?'disabled':''} onclick="openEvidence('${e.id}')"><span class="fileicon">${iconFor(e.type)}</span><div><b>${esc(e.title)}</b><p>${esc(e.teaser)}</p></div><span>›</span></button>`).join('')}</div>`:''}
...
function renderLeads(){
  return shell(`<div class="pagehead"><button class="back" onclick="go('home')">‹ Back</button><h1>Choose a lead</h1><p>Every table eventually receives the essential clues. Your case length determines how many side paths you must investigate.</p></div>
    <div class="cardlist">${state.game.leads.map((lead)=>`<div class="leadcard ${lead.completed?'complete':''}"><div><b>${esc(lead.title)}</b><p>${esc(lead.description)}</p></div><button class="btn ${lead.completed?'secondary':'primary'}" ${lead.completed||progress().paused?'disabled':''} onclick="completeLead('${lead.id}')">${lead.completed?'Completed':'Investigate'}</button></div>`).join('')}</div>`);
}
...
function renderMore(){
  return shell(`<div class="pagehead"><h1>More</h1><p>Manage your case and view event information.</p></div>
    <div class="menulist"><button onclick="togglePause()"><span>${progress().paused?'▶':'Ⅱ'}</span><div><b>${progress().paused?'Resume case':'Pause case'}</b><p>Your progress is always saved.</p></div><i>›</i></button>
    <button onclick="showLeaderboard()"><span>★</span><div><b>Leaderboard</b><p>Scores and team names only—never the culprit.</p></div><i>›</i></button>
...
function renderResults(){
  ...
  return shell(`<div class="resulthead"><span class="score">${a.score}</span><h1>${esc(a.rank)}</h1><p>Your accusation is locked.</p></div>
...
function renderModal(){
  ...
  if(state.modal.type==='evidence'){
    ...
    return `<div class="modalback" onclick="closeModal(event)"><section class="modal"><button class="modalclose" onclick="state.modal=null;render()">×</button><span class="eyebrow">${esc(e.type)} · ${esc(e.id)}</span><h1>${esc(e.title)}</h1><p class="leadtext">${esc(e.teaser)}</p>${e.facts?`<div class="factlist">${e.facts.map((f)=>`<div><span>◆</span><p>${esc(f)}</p></div>`).join('')}</div><div class="why"><b>Why it may matter</b><p>${esc(e.why)}</p></div>${hintCard()}<button class="btn primary wide" onclick="state.modal=null;go('home')">${esc(e.nextLabel||'Continue Investigation')}</button>`:`<button class="btn primary wide" onclick="viewEvidenceNow('${e.id}')" ${progress().paused?'disabled':''}>Examine Evidence</button>`}</section></div>`;
  }
  if(state.modal.type==='suspect'){
    ...
    return `<div class="modalback" onclick="closeModal(event)"><section class="modal"><button class="modalclose" onclick="state.modal=null;render()">×</button><div class="suspecthero"><div class="avatar large">${esc(s.initials)}</div><div><span class="eyebrow">Suspect profile</span><h1>${esc(s.name)}</h1><p>${esc(s.role)}</p></div></div><div class="why"><b>Known concern</b><p>${esc(s.publicMotive)}</p></div>
      ${next?`<section class="interview"><span class="eyebrow">${next.recommended?'Recommended follow-up':'Ask next'}</span><h2>${esc(next.text)}</h2><button class="btn primary wide" ${progress().paused?'disabled':''} onclick="askQuestion('${s.id}','${next.id}',${s.followups.some((f)=>f.id===next.id)})">Ask Question</button></section>`:s.questions.length?'<p class="goodmsg">All currently available questions have been asked.</p>':'<p class="muted">Interviews open after the murder is announced.</p>'}
...
function closeModal(event){if(event.target.classList.contains('modalback')){state.modal=null;render();}}

async function openEvidence(id){
  const e=evidenceById(id);if(!e)return toast('Evidence not available yet.');
  if(!e.viewed&&!progress().paused){await gameAction('view_evidence',{evidenceId:id},true);} state.modal={type:'evidence',id};render();
}
async function viewEvidenceNow(id){await gameAction('view_evidence',{evidenceId:id},true);state.modal={type:'evidence',id};render();}
async function openSuspect(id){const s=suspectById(id);if(!s)return; if(!s.opened&&!progress().paused)await gameAction('open_suspect',{suspectId:id},true);state.modal={type:'suspect',id};render();}
...
async function togglePause(){await gameAction('set_paused',{paused:!progress().paused});}
...
function renderHost(){
  ...
  app.innerHTML=`<div class="hostwrap"><div class="hosthead"><div><span class="eyebrow">${esc(d.session.code)}</span><h1>Host Dashboard</h1><p>${esc(d.session.title)}</p></div><button class="btn secondary" onclick="loadHost()">Refresh</button></div>
    <div class="hoststats"><div><b>${d.counts.registered}</b><span>Registered</span></div><div><b>${d.counts.playing}</b><span>Playing</span></div><div><b>${d.counts.paused}</b><span>Paused</span></div><div><b>${d.counts.help}</b><span>May need help</span></div><div><b>${d.counts.completed}</b><span>Completed</span></div></div>
...
async function boot(){
  ...
  try{state.config=await api(`/api/config?session=${encodeURIComponent(state.session)}`);if(state.config.session?.code)syncSession(state.config.session.code);}catch{}
...
function render(){if(new URLSearchParams(location.search).has('host'))return renderHost();if(!state.player||!state.game)return renderLanding();app.innerHTML=renderPlayer();}

Object.assign(window,{state,render,go,openFiles,openEvidence,viewEvidenceNow,openSuspect,askQuestion,openDeduction,submitDeduction,completeLead,useHint,togglePause,addNote,deleteNote,saveTheory,showLeaderboard,setJoinTab,joinIdentity,choosePlayMode,joinModeNext,registerPlayer,resumePlayer,logout,selectAccuse,nextAccuse,previousAccuse,submitAccusation,closeModal,hostLogin,loadHost,loadQR,toggleRegistration,showFixedVersion,saveVersionMode,resetPlayer});
boot();
setInterval(()=>{if(state.player&&state.game&&!state.accusation&&!progress().paused)gameAction('heartbeat',{},true).then(()=>{if(state.view==='home')render();}).catch(()=>{});},30000);
