...
async function createSession({ code, title, venue, pacingMode = 'logical', versionMode = 'rotating', fixedVersion = null }) {
  const row = {
    id: randomUUID(),
  ...
  if (hasPg) {
    try {
      const result = await pool.query(
        `INSERT INTO sessions(id,code,title,venue,pacing_mode,version_mode,fixed_version)
         VALUES($1,$2,$3,$4,$5,$6,$7) RETURNING *`,
        [row.id, row.code, row.title, row.venue, row.pacing_mode, row.version_mode, row.fixed_version],
      );
