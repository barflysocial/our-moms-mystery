...
async function completeQuickCase(token, version) {
  ...
  for (const id of ['E01','E02','E03','E04']) assert.equal((await action(token,'view_evidence',{evidenceId:id})).response.status,200);
...
(async () => {
  ...
  try {
    ...
    assert.equal(result.response.status,200); assert.equal(result.body.version,3); assert.equal(result.body.cases,4);
    assert.equal((await jsonRequest('/api/game')).response.status,401);

    const registrations=[];
    for (let i=0;i<4;i+=1) {
      const mobile=`22555520${String(i).padStart(2,'0')}`;
      result=await jsonRequest('/api/register',{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({sessionCode:'OURMOMS',name:`Detective ${i+1}`,mobile,pin:'1234',playMode:'individual',caseLength:'quick'})});
    ...
    assert.equal(result.response.status,200); assert.equal(result.body.accusation.score,98); assert.equal(result.body.accusation.solution.killer,solution.killerName);
    ...
    assert.equal((await action(second.token,'set_paused',{paused:true})).response.status,200);
    assert.equal((await action(second.token,'view_evidence',{evidenceId:'E01'})).response.status,200,'Already reviewed evidence remains available while paused');
    assert.equal((await action(second.token,'view_evidence',{evidenceId:'E02'})).response.status,409,'Paused cases cannot unlock new evidence');
    assert.equal((await action(second.token,'set_paused',{paused:false})).response.status,200);
    ...
    result=await jsonRequest('/api/host/session',{method:'POST',headers:{'content-type':'application/json',authorization:`Bearer ${hostToken}`},body:JSON.stringify({code:'OURMOMS',title:'Duplicate',venue:'Duplicate'})});
    ...
    console.log('Smoke test passed: four rotating cases, guided progression, pause rules, protected solutions, immutable scoring, sanitized host data, and QR access.');
