...
(async()=>{
 ...
 try{
  ...
  await action(token,'view_briefing',{briefingId:'B_C00'});
  await action(token,'view_evidence',{evidenceId:'E01'}); await action(token,'view_evidence',{evidenceId:'E02'});
  await action(token,'view_briefing',{briefingId:'B_C01'});
  ...
  const appSource=fs.readFileSync(path.join(root,'public','app.js'),'utf8'); assert(appSource.includes("localStorage.getItem('reunionLastSession')")); assert(appSource.includes('Tap to Reveal Solution')); assert(appSource.includes('Investigation Lobby')); assert(appSource.includes('saveSchedule')); assert(appSource.includes('Detective Briefings')); assert(appSource.includes('investigatorLabel')); assert(appSource.includes('investigatorInitials')); assert(appSource.includes('Play narration')); assert.equal((appSource.match(/pattern=\"\[0-9\]\{4\}\"/g)||[]).length,2,'Both player PIN fields must use a browser-safe numeric pattern'); assert(!appSource.includes('pattern=\"\\d{4}\"'),'JavaScript-generated HTML must not use an unescaped \d PIN pattern');
  const sw=fs.readFileSync(path.join(root,'public','sw.js'),'utf8'); assert(sw.includes('reunion-v3-3-lobby')); assert(sw.includes('caches.delete'));
  for(const [id,c] of Object.entries(cases.cases)){assert.equal(c.suspects.length,12,`Case ${id} suspect count`);assert.equal(c.evidence.length,18,`Case ${id} evidence count`);assert.equal(c.deductions.length,3);assert(c.solution.killerId&&c.solution.weaponId&&c.solution.clueId);assert.equal(c.briefings.length,6);assert(c.briefings.every((b)=>b.speaker==='Player detective'&&b.paragraphs.length>=3));}
  console.log('Regression test passed: personalized detective identities, synchronized scheduled lobby, detective briefings, soft pacing, PIN throttling, PWA session retention, and four authored cases.');
